...
- [x] 1. Project scaffold (Vite + Three.js + Rapier.js)
  - [x] 1.1 Initialise Vite project with `index.html`, `package.json`, and `src/main.js`
    - Add Three.js (latest stable) and `@dimforge/rapier3d-compat` as dependencies
    - Configure Vite for ES module development
    - _Requirements: 1.1, 1.3_

  - [x] 1.2 Create `src/renderer/SceneBuilder.js` with WebGPURenderer initialisation
  ...
  - [x] 1.3 Create Rapier.js initialisation in `src/physics/RigidBodies.js`
  ...
  - [x] 1.4 Wire `src/main.js` entry point
...
- [x] 2. Port prototype into module structure (WebGLRenderer first)
  - [x] 2.1 Create `src/physics/WaterSimCPU.js` — extract CPU water simulation from prototype
  ...
  - [x] 2.2 Create `src/renderer/TerrainMesh.js` — terrain PlaneGeometry with vertex displacement
  ...
  - [x] 2.3 Create `src/physics/WaterSim.js` — facade that delegates to GPU or CPU implementation
    - Detect WebGPU availability; select WaterSimGPU or WaterSimCPU
    - Expose unified API surface for all consumers
    - _Requirements: 4.2_

  - [x] 2.4 Create the UnifiedGrid data structure
  ...
  - [x] 2.5 Add water surface rendering
  ...
  - [x] 2.6 Integrate terrain mesh, water sim (CPU), and camera orbit into main.js
...
- [x] 3. Checkpoint — Confirm prototype port
  - Ensure terrain renders, water flows downhill on CPU sim, camera orbits freely. Ask the user if questions arise.

- [x] 4. GPU water simulation (WebGPU compute shaders)
  - [x] 4.1 Create `src/shaders/water-sim.wgsl` — SWE Virtual Pipes compute shader
  ...
  - [x] 4.2 Create `src/physics/WaterSimGPU.js` — WebGPU compute pipeline wrapper
  ...
  - [x] 4.3 Switch renderer from WebGLRenderer to WebGPURenderer in SceneBuilder
    - Update SceneBuilder to use Three.js WebGPURenderer
    - Ensure terrain mesh and water surface still render correctly
    - _Requirements: 1.1_

  - [x] 4.4 Create `src/shaders/water-render.wgsl` — water surface vertex/fragment shader
    - Vertex shader displaces water plane from depth buffer
    - Fragment shader applies depth-based blue colouring
    - _Requirements: 18.2, 18.3_

  - [x] 4.5 Wire WaterSimGPU into the main loop via WaterSim facade
...
- [x] 5. Checkpoint — GPU water simulation verified
...
- [x] 6. Rigid body integration (Rapier.js)
  - [x] 6.1 Implement full RigidBodies module in `src/physics/RigidBodies.js`
  ...
  - [x] 6.2 Sync Rapier body transforms to Three.js meshes each frame
  ...
  - [x] 6.3 Implement block stacking and toppling
...
- [x] 7. Water–rigid body coupling
  - [x] 7.1 Implement cell blocking from rigid body AABBs
  ...
  - [x] 7.2 Implement water deflection around rigid bodies
    - Confirm water visibly flows around a placed stone block
    - Confirm removing a block allows water to fill the gap
    - _Requirements: 6.2, 6.3_

  - [x] 7.3 Implement "support lost" event from erosion to rigid bodies
...
- [x] 8. Checkpoint — Physics layers coupled
...
- [x] 9. Material system
  - [x] 9.1 Create `src/game/Materials.js` — material definitions and placement logic
  ...
  - [x] 9.2 Implement erosion in `src/physics/Erosion.js`
  ...
  - [x] 9.3 Implement timber degradation during Flood_Phase
...
- [x] 10. House objects and flood detection
  - [x] 10.1 Create `src/game/Houses.js` — house mesh creation and flood detection
  ...
  - [x] 10.2 Implement flood detection with hysteresis in `src/physics/FloodDetection.js`
  ...
  - [x] 10.3 Implement house visual state transitions
...
- [x] 11. Checkpoint — Materials, erosion, houses, flood detection working
...
- [x] 12. Game loop and phase state machine
  - [x] 12.1 Create `src/game/GameLoop.js` — three-phase state machine
  ...
  - [x] 12.2 Implement win/loss evaluation
  ...
  - [x] 12.3 Wire GameLoop into main.js
    - GameLoop controls WaterSim start/stop, phase UI, and resolution display
    - _Requirements: 10.1_

- [x] 13. Resource budget system
  - [x] 13.1 Create `src/game/ResourceBudget.js` — per-material unit tracking
  ...
  - [x] 13.2 Create budget display UI
    - Show remaining units per material type in construction toolbar
    - _Requirements: 11.5_

- [x] 14. Undo system
  - [x] 14.1 Create `src/game/UndoSystem.js` — snapshot stack for construction phase
  ...
  - [x] 14.2 Add undo UI button and keyboard shortcut
    - Undo button: 48×48px minimum in construction toolbar
    - Ctrl+Z / Cmd+Z keyboard shortcut
    - _Requirements: 12.6, 12.7_

- [x] 15. Level data loading
  - [x] 15.1 Create `src/game/Level.js` — JSON level loader
  ...
  - [x] 15.2 Author `src/levels/level-01.json` — Level 1 "The Basin"
...
- [x] 17. Touch input
  - [x] 17.1 Create `src/input/InputHandler.js` — Pointer Events unified input
  ...
  - [x] 17.2 Ensure all UI elements meet 48×48px minimum touch target
    - Audit all buttons: flood/drain, material selector, undo
    - _Requirements: 13.8_

- [x] 18. Desktop keyboard and mouse input
  - [x] 18.1 Add WASD camera controls with acceleration and damping
...
- [x] 19. Checkpoint — Input systems complete
...
- [x] 20. Visual polish and particle effects
  - [x] 20.1 Implement particle system in `src/renderer/ParticleSystem.js`
...
- [x] 21. Final checkpoint — Level 1 complete and polished
